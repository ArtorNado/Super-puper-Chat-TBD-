...
class ThemeFragment : BaseFragment<ThemeViewModel>(){
    ...
    override fun onCreateView(
        ...
        themeSharedPreferences?.getInt(Companion.KEY_THEME, 1)?.let { AppCompatDelegate.setDefaultNightMode(it) }
    ...
    override fun initClickListeners() {
        ...
        btn_theme.setOnClickListener{
            if (AppCompatDelegate.getDefaultNightMode() == AppCompatDelegate.MODE_NIGHT_YES) {
                ...
                themeSharedPreferences?.edit()?.putInt(Companion.KEY_THEME, AppCompatDelegate.MODE_NIGHT_NO)?.apply()
            } else {
                AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_YES)
                themeSharedPreferences?.edit()?.putInt(Companion.KEY_THEME, AppCompatDelegate.MODE_NIGHT_YES)?.apply()
            }
        }
    }
